...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv_file'])) {
    try {
        ...
        if (!empty($missingColumns)) {
            $templateUrl = $importType === 'products' 
                ? 'templates/products_template.csv' 
                : 'templates/movements_template.csv';
            
            throw new Exception(
                "Missing required columns: " . implode(", ", $missingColumns) . "<br>" .
                "Please download the template file and use it as a reference: " .
                "<a href='$templateUrl' download class='btn btn-sm btn-primary mt-2'>" .
                "<i class='fas fa-download me-2'></i>Download Template</a><br>" .
                "Note: Make sure to keep the column headers exactly as shown in the template."
            );
        }
        ...
        if ($importType === 'products') {
            ...
            $product_stmt = $db->prepare("INSERT INTO products (name, category_id, stock_quantity, price) 
        ...
        while (($data = fgetcsv($handle)) !== FALSE) {
            ...
            try {
                ...
                if (count($data) < count($requiredColumns)) {
                    throw new Exception("Invalid number of columns. Expected " . count($requiredColumns) . 
                                      " columns but found " . count($data));
                }
                
                if ($importType === 'products') {
                    // Process product import
                    $name = trim($data[$indices['product_name']]);
                    $category = trim($data[$indices['category']]);
                    $quantity = (int)$data[$indices['quantity']];
                    ...
                    if ($check_stmt->fetch()) {
                        $stats['warnings'][] = "Row $row: Product '$name' already exists - skipped";
                        $stats['skipped_count']++;
                        continue;
                    }
                    ...
                    // Insert product
                    $product_stmt->execute([$name, $category, $quantity, $price]);
                } else {
                    ...
                    if ($type === 'out' && ($product['stock_quantity'] + $quantity_change) < 0) {
                        $stats['warnings'][] = "Row $row: Insufficient stock for product ID $product_id - skipped";
                        $stats['skipped_count']++;
                        continue;
                    }
        ...
        $message[] = "- Successfully Imported: " . $stats['success_count'];
        
        if ($importType === 'products' && $stats['categories_created'] > 0) {
            $message[] = "- New Categories Created: " . $stats['categories_created'];
        }
        
        if ($stats['skipped_count'] > 0) {
            $message[] = "- Skipped: " . $stats['skipped_count'];
            $message[] = "Warnings:";
        ...
        if ($stats['error_count'] > 0) {
            $message[] = "- Errors: " . $stats['error_count'];
        ...
        if ($stats['error_count'] === 0) {
            $database->commit();
            $_SESSION['success'] = implode("<br>", $message);
        } else {
            $database->rollBack();
                    ...
                    <form id="uploadCsvForm" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <div id="productsTemplate" class="import-template">
                                ...
                                <p>Required columns: product_name, category, quantity, price</p>
